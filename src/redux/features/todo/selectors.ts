...
// Base Selectors
export const selectTodos = (state: { todo: todoState }): Todo[] =>
  state.todo.todos;
export const selectFilter = (state: { todo: todoState }): Filter =>
  state.todo.filter;
...
export const selectFilteredAndSearchedTodos = createSelector(
  ...
  (todos, filter, searchTerm): Todo[] => {
    return todos
      ...
      .filter((todo: Todo) => todo.text.toLowerCase().includes(searchTerm));
  },
);
